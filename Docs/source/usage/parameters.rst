...
* ``<diag_name>.fields_to_plot`` (list of `strings`, optional)
    ...
    ``P_<species_name>`` is the pressure tensor, output as 6 components (``Pxx``, ``Pxy``, ``Pxz``, ``Pyy``, ``Pyz``, ``Pzz``) in eV.
    The trace ``(Pxx + Pyy + Pzz) / 3`` equals the scalar temperature ``T``.
    ``Q_<species_name>`` is the heat flux vector, output as 3 components (``Qx``, ``Qy``, ``Qz``) in eV times the speed of light.
