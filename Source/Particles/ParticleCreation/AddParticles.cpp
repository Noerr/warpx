...
#pragma omp critical (add_plasma_nextid)
        ...
        [=] AMREX_GPU_DEVICE (int i, int j, int k, amrex::RandomEngine const& engine) noexcept
        {
            ...
            // consecutive i_part indices shares one physical position (the
            // position injector is invoked only when m_vel_idx == 0) and the
            // momentum injector receives the per-group velocity-sample index
            // m_vel_idx. M_vel == 1 reproduces the historical behaviour of one
            // position per particle exactly.
            XDim3 r_cached;
